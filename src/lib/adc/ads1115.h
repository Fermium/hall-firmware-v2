/*!
   ...
   \note Partially ported from Adafruit arduino library https://github.com/adafruit/Adafruit_ADS1X15
   \note ADS115 datasheet: http://www.ti.com/lit/ds/symlink/ads1114.pdf
*/
...
#define ADS1115_REG_CONFIG_OS_MASK      (0x8000)  /*!< OS Register (status of conversion) mask */
...
#define ADS1115_REG_CONFIG_MUX_MASK     (0x7000)  /*!< MUX config register mask */
...
#define ADS1115_REG_CONFIG_PGA_MASK     (0x0E00)  /*!< PGA config register mask */
...
#define ADS1115_REG_CONFIG_MODE_MASK    (0x0100)  /*!< MODE register mask */
...
#define ADS1115_REG_CONFIG_DR_MASK      (0x00E0)  /*!< Data rate (sps) register mask */
...
const int sp[8]={8,16,32,64,128,250,475,860}; /*!< Array that converts froma 0-7 channel notation to the samples per second of each channel */


#define ADS1115_REG_CONFIG_CMODE_MASK   (0x0010)  /*!< Comparator configuration register mask */
...
#define ADS1115_REG_CONFIG_CPOL_MASK    (0x0008)  /*!< ALERT/RDY pin config register mask */
...
const float ADS1115_RANGE_CORRECTION[6] = {7.8125e-06, 1.5625e-05, 3.125e-05, 6.25e-05, 0.000125, 0.0001875}; /*!< Values to multiply each channel raw measurement to, to obtain the correct voltage considering the PGA */

#define ADS1115_ADDR_GND            (0b10010000)  /*!< ADS1115 address when ADDR pin is connected to GND */
#define ADS1115_ADDR_VDD            (0b10010010)  /*!< ADS1115 address when ADDR pin is connected to VDD */
#define ADS1115_ADDR_SDA            (0b10010100)  /*!< ADS1115 address when ADDR pin is connected to SDA */
#define ADS1115_ADDR_SCL            (0b10010110)  /*!< ADS1115 address when ADDR pin is connected to SCL */
  ...
  class ADS1115
  {
    ...
    public  :
	   ...
	   /*!
	      ...
	      \return the I2C address of the ADS115
	   */
       uint8_t getaddress(){ return this->address; }
	   /*!
	      \brief Set I2C address
	      \param address The I2C address of the ADS115
	   */
       void setaddress(uint8_t address){ this->address=address; }
	   /*!
		    \brief Get the set PGA range for a channel
		    \param channel the channel
        \return the range for the selected channel
	   */
       uint8_t getrange(uint8_t channel){ return this->range[channel]; }
       /*!
          \brief Set PGA range for specific channel
          \param channel the channel from 0 to 7
          \param range the PGA range to be set (check the DEFINE macros)
       */
       ...
       /*!
          \brief Get the samples per seconds of the ADS115
          \return samples per seconds: 8,16,32,64,128,250,475, or 860
       */
       uint16_t getsps(){ return sp[(this->config_reg_sps>>4)/2]; }
       /*!
          \brief Get the sampling period
          \return sampling period in milliseconds
       */
       uint16_t getsperiod_ms(){ return (float) 1/sp[(this->config_reg_sps>>4)/2]*1000; }
